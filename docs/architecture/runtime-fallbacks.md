...
**Risk:** Esecuzione della matrice 6×6 (6 capability profile × 6
...
| Probability | Low |
| Impact | Medium |
| Mitigation type | Manual `workflow_dispatch` trigger + 4-state skip contract + integration test on public helpers |
| PR candidacy | candidate |
| Upstream issue | D109 (trigger), D022 (no spawn on push) |
...
3. `npm ci` (install delle deps del repo).
...
| --- | --- | --- | --- |
| Skip esplicito | `GSD_E2E_SKIP=1` | 0 | L'utente richiede skip; nessuna matrice. |
| Skip implicito | `gsd` non su PATH | 2 | `findGsd` ritorna `null`; la CI deve aver installato `@opengsd/gsd-pi@latest`. |
| OK | tutti scenari match | 0 | Tutti i 36 scenari hanno `observedTier === expectedTier`. |
...
rende esplicita la semantica di on-demand.
...
binario reale) sono short-circuitate con `skipped: true, exitCode: 0`
senza spawn.
